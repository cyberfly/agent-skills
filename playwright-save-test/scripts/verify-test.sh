...
if [ -z "$CONFIG_FOUND" ]; then
  echo "Warning: no playwright.config.* found in $(pwd)." >&2
  echo "Run 'npm init playwright@latest' to set up Playwright Test, or move the spec into a project that has it." >&2
fi
